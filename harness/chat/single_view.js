...
async function run() {
  try {
    ...
    console.log('chat_single_view: skipped (relay not reachable)')
    process.exit(0)
  }
  ...
  const pt = Buffer.from('chat-payload')
  const { enc, tag } = aesGcmEncrypt(mk, iv, pt, aad)

  const post = await fetch(`${RELAY_URL}/relay`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ iv: iv.toString('base64'), data: enc.toString('base64'), tag: tag.toString('base64'), ttl_ms: 30000 })
  })
  ...
  console.log('chat_single_view: ok')
}
